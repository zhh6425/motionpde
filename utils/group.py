...
class QueryAndGroup(nn.Module):
    def __init__(self, radius: float, nsample: int,
        ...
        self.use_gaussion_density = use_gaussion_density
    ...
    def forward(self, query_xyz: torch.Tensor, support_xyz: torch.Tensor, features: torch.Tensor = None, time: torch.Tensor = None) -> Tuple[
        torch.Tensor]:
        """
        ...
        if time is not None:
            time_trans = time.transpose(1, 2).contiguous()
            grouped_time = grouping_operation(time_trans, idx)  # (B, 1, npoint, nsample)

        grouped_features = grouping_operation(features, idx) if features is not None else None
        if self.use_gaussion_density:
        ...
        if time is not None:
            return grouped_xyz, grouped_features, grouped_time
        else:
...
class KNNGroup(nn.Module):
    ...
    def forward(self, query_xyz: torch.Tensor, support_xyz: torch.Tensor, features: torch.Tensor = None) -> Tuple[
        torch.Tensor]:
        """
        :param support_xyz: (B, N, 3) xyz coordinates of the features
        :param query_xyz: (B, npoint, 3) centroids
        :param features: (B, C, N) descriptors of the features
        :return:
            new_features: (B, 3 + C, npoint, nsample)
        """
